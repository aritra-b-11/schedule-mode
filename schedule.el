...
(defun schedule-derive-planned-start-end-date-with-owner ()
  ...
  (interactive)
  (let* (owner-table-pos schedule-table-pos each-owner (owner-list '()) start end start-date)
    (setq owner-list (schedule-construct-owner-list))
    (search-backward "#+CAPTION: Owner Effort Table" nil t)
    (search-forward "#+CAPTION: Owner Effort Table" nil t)
    (search-forward "#+BEGIN_TABLE" nil t)
    (search-forward-regexp (concat "|[ ]+" schedule-owner-effort-table-name "[ ]+|"))
    ...
    (dolist (each-owner owner-list)
      ...
      (search-backward "#+CAPTION: Effort Estimation Table" nil t)
      ;; (previous-line)
      (setq start (point))
      (search-forward "#+CAPTION: Schedule Estimation Table" nil t)
      (search-forward "#+END_TABLE")
	...
	(schedule-go-to-planning-start-column)
	(schedule-add-end-date-adjusted start-date "1")
	;; (insert start-date)
	(org-shifttab)
	(schedule-effort-adjust-add-planned-end-date)
	(org-cycle)
