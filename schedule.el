...
(defun schedule-construct-work-list-in-schedule-table ()
  ...
  (let* ((work-list-from-schedule '()) pos start end each-work)
    ...
    (setq end (point))
    (goto-char start)
    (while (search-forward-regexp "\\([a-zA-Z0-9._]+[ ]+[(]*[ a-zA-Z0-9._]+[)]*\\)" nil t)
...
(defun schedule-allocate-work-with-mile-stone ()
  ...
  (interactive)
  (let* (mile-stone-list block-work-assoc-list task-list choice mile-stone-pos)
	...
	(if (equal choice "y")
	    (progn
	      ...
	      (setq block-work-assoc-list (remove* each-work-task block-work-assoc-list))
	      )
	  )
	(if (equal choice "n")
	    (progn
